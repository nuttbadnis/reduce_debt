...
Public Class Cls_Data
    ...
    Public Function GetConnectionString() As String
        ...
        strConn = Configuration.WebConfigurationManager.ConnectionStrings("FeasibilityConnectionString2").ConnectionString.ToString
        Return strConn
    End Function
...
#Region "DataNonTransaction"
    ...
    'Public Sub SetSelectOption(t As System.Web.UI.HtmlControls.HtmlSelect, ByVal CmdText As String, ByVal TextField As String, ByVal ValueField As String)
    '    Dim value_option = ""
    '    Try
    '        Dim DT As DataTable = GetDataTable(CmdText)
    ...
    '                t.Items.Add(New ListItem("dd", "dd"))
    '            Next
    '        End If
    '    Catch ex As Exception
    '        Err.Raise(Err.Number, , ex.Message)
    ...
    Public Function SetSelectOption(t As System.Web.UI.HtmlControls.HtmlSelect, ByVal CmdText As String, ByVal TextField As String, ByVal ValueField As String) As String
        Dim value_option = "test"
        Try
            Dim DT As DataTable = GetDataTable(CmdText)
            If DT.Rows.Count > 0 Then
                value_option = "yes"
...
#Region "Utility"
    ...
    Public Function GetDateNow() As String
        Try
            Dim vDate As String
            Dim curCulture As System.Globalization.CultureInfo
    ...
    Public Function rpSpecialChars(ByVal profileString As String) As String
        ...
        profileString = profileString.Replace("&quot;", """")
        profileString = profileString.Replace("&apos;", "'")
        profileString = profileString.Replace("&reg;", "?")
        Return profileString
    End Function

    Public Function RO_User(ByVal uemail As String) As String
        Dim strSql As String = "select *  from RO_Director where RO_email = '" & uemail & "' and status='1' "
        Dim DT As New DataTable
        DT = GetDataTable(strSql)
        If DT.Rows.Count > 0 Then
            Return DT.Rows(0).Item("RO")
        Else
            Return "0"
        End If
    End Function

    Public Function Cluster_User(ByVal uemail As String) As String
        Dim strSql As String = "select *  from Cluster where Cluster_email = '" & uemail & "' and status='1' "
        Dim DT As New DataTable
        DT = GetDataTable(strSql)
        If DT.Rows.Count > 0 Then
            Return DT.Rows(0).Item("Cluster")
        Else
            Return "0"
        End If
    End Function

    Public Function Branch_User(ByVal uemail As String) As String
        Dim strSql As String = "select *  from UserBranch where Login_name = '" & uemail & "' "
        Dim DT As New DataTable
        DT = GetDataTable(strSql)
        If DT.Rows.Count > 0 Then
            If DT.Rows(0).Item("RO") <> "ALL" Then
                If DT.Rows(0).Item("Cluster") <> "ALL" Then
                    Return DT.Rows(0).Item("Cluster")
                Else
                    Return DT.Rows(0).Item("RO")
                End If
    ...
    Public Function Check_Branch_User(ByVal uemail As String) As String
        Dim strSql As String = "select *  from UserBranch where Login_name = '" & uemail & "' and Cluster <> 'ALL' "
        Dim DT As New DataTable
        DT = GetDataTable(strSql)
        If DT.Rows.Count > 0 Then
            Return 1

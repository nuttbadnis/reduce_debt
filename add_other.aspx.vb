...
Partial Class add_other
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Page.IsPostBack Then
            ...
            C.ExecuteNonQuery("delete from tmpList_OTHER where CreateBy = '" + Session("uemail") + "' and Document_No is null ")
            C.ExecuteNonQuery("insert into tmpList_OTHER select OTHER_Name, Cost_perUnit, Number, Cost, Initial_Cost_perUnit, CreateBy, CreateDate, UpdateBy, UpdateDate, Document_No from List_OTHER where CreateBy = '" + Session("uemail") + "' and Document_No is null ")
            DT_List = C.GetDataTable("select * from tmpList_OTHER where CreateBy = '" + Session("uemail") + "' and Document_No is null ")
        ...
        If IsNumeric(TextBox4.Text) = True Then
            ...
            If chkOther.Checked = True Then
                ...
                Else
                    ...
                    strSql = "insert into tmpList_OTHER (OTHER_Name, Cost_perUnit, Number, Cost, Initial_Cost_perUnit, CreateBy, CreateDate) values('" + C.rpSpecialChars(txtOther.Text.ToString) + "','0.00','" + TextBox4.Text + "','0.00','0.00','" + Session("uemail") + "',getdate()) "
                    C.ExecuteNonQuery(strSql)
                    DT_List = C.GetDataTable("select * from tmpList_OTHER where CreateBy = '" + Session("uemail") + "' and Document_No is null ")
                    GridView1.DataSource = DT_List
                    GridView1.DataBind()
            ...
            Else
                ...
                If DT.Rows.Count > 0 Then
                    ...
                    strSql = "insert into tmpList_OTHER (OTHER_Name, Cost_perUnit, Number, Cost, Initial_Cost_perUnit, CreateBy, CreateDate) values('" + C.rpSpecialChars(DT.Rows(0).Item("OTHER_Name").ToString) + "','" + DT.Rows(0).Item("OTHER_Cost").ToString + "','" + TextBox4.Text + "','" + (CDbl(DT.Rows(0).Item("OTHER_Cost")) * TextBox4.Text).ToString + "','" + DT.Rows(0).Item("OTHER_Cost").ToString + "','" + Session("uemail") + "',getdate()) "
                    C.ExecuteNonQuery(strSql)
                    DT_List = C.GetDataTable("select * from tmpList_OTHER where CreateBy = '" + Session("uemail") + "' and Document_No is null ")
                    GridView1.DataSource = DT_List
                    GridView1.DataBind()
    ...
    Private Sub SaveData(ByVal SaveOrNext As String)
        ...
        If GridView1.Rows.Count > 0 Then
            ...
            Next
        End If
        C.ExecuteNonQuery("delete from List_OTHER where CreateBy = '" + Session("uemail") + "' and Document_No is null ")
        Try
            C.ExecuteNonQuery("insert into List_OTHER (OTHER_Name, Cost_perUnit, Number, Cost, Initial_Cost_perUnit, Status, CreateBy, CreateDate, UpdateBy, UpdateDate, Document_No) select OTHER_Name, Cost_perUnit, Number, Cost, Initial_Cost_perUnit, '1', CreateBy, CreateDate, UpdateBy, UpdateDate, Document_No from tmpList_OTHER where CreateBy = '" + Session("uemail") + "' and Document_No is null ")
            If SaveOrNext = "Next" Then
                ClientScript.RegisterStartupScript(Page.GetType, "success_insert", "AlertSuccess('บันทึกข้อมูลได้สำเร็จ',function(){ window.location = 'add_management.aspx?menu=create'; });", True)
            Else

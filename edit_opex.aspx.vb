...
Partial Class edit_opex
        ...
        If Not Page.IsPostBack Then
            ...
            If dt_check.Rows.Count > 0 Then
                ...
                If (Session("Login_permission") = "administrator" Or (dt_check.Rows(0).Item("request_status").ToString = "55" And Session("Login_permission") = "inspector") Or (dt_check.Rows(0).Item("request_status").ToString = "110" And dt_check.Rows(0).Item("CreateBy").ToString = Session("uemail"))) Then
                    ...
                    C.ExecuteNonQuery("delete from tmpList_OPEX where Document_No = '" + xRequest_id + "' -- and CreateBy = '" + Session("uemail") + "' ")
                    C.ExecuteNonQuery("insert into tmpList_OPEX select OPEX_Name, Cost_perUnit, Number, Cost, Initial_Cost_perUnit, CreateBy, CreateDate, UpdateBy, UpdateDate, Document_No from List_OPEX where Document_No = '" + xRequest_id + "' and Status = '1' -- and CreateBy = '" + Session("uemail") + "' ")
                    DT_List = C.GetDataTable("select * from tmpList_OPEX where Document_No = '" + xRequest_id + "' -- and CreateBy = '" + Session("uemail") + "' ")
        ...
        If IsNumeric(TextBox4.Text) = True Then
            ...
            If chkOther.Checked = True Then
                ...
                Else
                    ...
                    strSql = "insert into tmpList_OPEX (OPEX_Name, Cost_perUnit, Number, Cost, Initial_Cost_perUnit, CreateBy, CreateDate, Document_No) values('" + C.rpSpecialChars(txtOther.Text.ToString) + "','0.00','" + TextBox4.Text + "','0.00','0.00','" + Session("uemail") + "',getdate(),'" + xRequest_id + "') "
                    C.ExecuteNonQuery(strSql)
                    DT_List = C.GetDataTable("select * from tmpList_OPEX where Document_No = '" + xRequest_id + "' -- and CreateBy = '" + Session("uemail") + "' ")
                    GridView1.DataSource = DT_List
                    GridView1.DataBind()
            ...
            Else
                ...
                If DT.Rows.Count > 0 Then
                    ...
                    strSql = "insert into tmpList_OPEX (OPEX_Name, Cost_perUnit, Number, Cost, Initial_Cost_perUnit, CreateBy, CreateDate, Document_No) values('" + C.rpSpecialChars(DT.Rows(0).Item("OPEX_Name").ToString) + "','" + DT.Rows(0).Item("OPEX_Cost").ToString + "','" + TextBox4.Text + "','" + (CDbl(DT.Rows(0).Item("OPEX_Cost")) * CInt(TextBox4.Text)).ToString + "','" + DT.Rows(0).Item("OPEX_Cost").ToString + "','" + Session("uemail") + "',getdate(),'" + xRequest_id + "') "
                    C.ExecuteNonQuery(strSql)
                    DT_List = C.GetDataTable("select * from tmpList_OPEX where Document_No = '" + xRequest_id + "' -- and CreateBy = '" + Session("uemail") + "' ")
                    GridView1.DataSource = DT_List
                    GridView1.DataBind()
    ...
    Private Sub SaveData(ByVal SaveOrNext As String)
        ...
        End If
        Try
            C.ExecuteNonQuery("Update List_OPEX set Status = 'A', UpdateBy = '" + Session("uemail") + "', UpdateDate = getdate() where Document_No = '" + xRequest_id + "' and Status = '1' -- and CreateBy = '" + Session("uemail") + "' ")
            C.ExecuteNonQuery("insert into List_OPEX (OPEX_Name, Cost_perUnit, Number, Cost, Initial_Cost_perUnit, Status, CreateBy, CreateDate, UpdateBy, UpdateDate, Document_No) select OPEX_Name, Cost_perUnit, Number, Cost, Initial_Cost_perUnit, '1', '" + Session("uemail") + "', getdate(), '" + Session("uemail") + "', getdate(), Document_No from tmpList_OPEX where Document_No = '" + xRequest_id + "' -- and CreateBy = '" + Session("uemail") + "' ")
            ...
            If SaveOrNext = "Next" Then
                ClientScript.RegisterStartupScript(Page.GetType, "success_insert", "AlertSuccess('อัพเดทข้อมูลได้สำเร็จ', function(){ window.location = 'edit_other.aspx?request_id=" & Request.QueryString("request_id") & "'; });", True)
            Else

...
Partial Class check_status_list
    ...
    Dim con As New SqlConnection(ConfigurationManager.ConnectionStrings("FeasibilityConnectionString").ConnectionString)
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Page.IsPostBack Then
            ...
            ShowData(num)
        Else
            ClientScript.RegisterStartupScript(Page.GetType, "test_postback", "console.log('postback');loader_data();", True)
    ...
    '        End If
    '        'cmd = New SqlCommand("SP_Feasibility_Document", con)
    ...
    Protected Function rowCount() As Integer
        ...
        Dim cmd As New SqlCommand("SP_Feasibility_Document_Count", con)
        cmd.CommandType = CommandType.StoredProcedure
        Try
            con.Open()
    ...
    Protected Sub btnLoadMore_Click(sender As Object, e As System.EventArgs) Handles btnLoadMore.Click
        ...
        Dim numVal As Integer = Convert.ToInt32(ViewState("num")) + num
        'Response.Write(ViewState("num"))

        'Now pass numVal whose value is 4 to the BindGridView function to get 4 rows.
        'BindGridView(numVal)
        Dim strsql As String = ""
        'If numVal > 0 And numVal <> 50 Then
            'strsql = "select top (" & numVal & ") "
        'Else
            strsql = "select "
        'End If
        strsql = strsql + "" + ViewState("query")
    ...
    Public Function GetData(ByVal TopRow As Integer) As DataTable
        Dim DT_List As New DataTable
        Dim strSql As String
        strSql = "select top (" & TopRow & ") doc.Document_No, CONVERT(varchar(10),name.Document_Date,103) Document_Date " + vbCr
    ...
    Public Sub ShowData(Optional ByVal numOfRows As Integer = 999)
        Dim DT_List As DataTable
        Dim limit_strSql As String
        Dim strSql As String = ""
        ...
        If numOfRows > 0 And numOfRows <> 999 Then
            limit_strSql = "select top (" & numOfRows & ") "
        Else
            limit_strSql = "select "
        End If
        strSql += " doc.Document_No, CONVERT(varchar(10),name.Document_Date,103) Document_Date " + vbCr
        strSql += ", name.Area, name.Cluster, name.Project_Name, name.Customer_Name, name.Project_Code, isnull(name.Get_Job,'0') 'Get_Job' " + vbCr
        strSql += ", case doc.request_status when 100 then 'อนุมัติ' when 105 then 'ไม่อนุมัติ' when 50 then 'รอพิจารณา' when 55 then 'รอพิจารณา' else re.status_name end 'status_name', doc.CreateBy " + vbCr
        strSql += "from FeasibilityDocument doc inner join List_ProjectName name on doc.Document_No = name.Document_No " + vbCr
        strSql += "left outer join request_status re on doc.request_status   = re.status_id " + vbCr
        ...
        strSql += " order by doc.CreateDate desc "
        'Response.Write(strSql)
        ViewState("query") = strSql
        strSql = limit_strSql + "" + strSql
        Dim DT As New DataTable
        DT = C.GetDataTable("select * from Permission where Permission_Type = '" & Session("Login_permission") & "'")
        'If Session("Login_permission") = "user" Or Session("Login_permission") = "administrator" Or Session("Login_permission") = "approve" Or Session("Login_permission") = "approve_ro" Or Session("Login_permission") = "approve_cluster" Or Session("Login_permission") = "presale" Then
        If DT.Rows.Count > 0 Then

...
Partial Class project_name
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not Page.IsPostBack Then
            ...
            strSql += ", Detail_Project, Detail_Service, SLA, isnull(SLA_File,'') 'SLA_File', isnull(Doc_File,'') 'Doc_File', MTTR, Monitor_Date, Monitor_Time, Fine, isnull(Fine_File,'') 'Fine_File', id_List " + vbCr
            ...
            If DT_List.Rows.Count > 0 Then
                ...
                DT_casst = C.GetDataTable("select * from tmpList_CAsst where CreateBy='" & hide_user.Value & "' and isnull(ProjectName_id, '') = '" & hide_doc_no.Value & "' ")
                If DT_casst.Rows.Count = 0 Then
                    ...
                    strSql_casst_cluster += "inner join dbo.Cluster cl on br.Cluster = cl.Cluster and cl.Status = '1' " + vbCr
                    ...
                    If DT_casst_cluster.Rows.Count > 0 Then
                        strSql_casst_cluster = "insert into tmpList_CAsst(Customer_Assistant_ID, Customer_Assistant_Name, Customer_Assistant_Email, Area, Cluster, Cluster_Name, Cluster_Email, CreateBy, CreateDate, ProjectName_id) " + vbCr
                        strSql_casst_cluster += "values ('" & Session("uid") & "','" & Session("ufullname") & "','" & hide_user.Value & "@jasmine.com','" & DT_casst_cluster.Rows(0).Item("Area") & "','" & DT_casst_cluster.Rows(0).Item("Cluster") & "','" & DT_casst_cluster.Rows(0).Item("Cluster_Name") & "','" & DT_casst_cluster.Rows(0).Item("Cluster_Email") & "','" & hide_user.Value & "',getdate(),'" & hide_doc_no.Value & "')"
                        C.ExecuteNonQuery(strSql_casst_cluster)
                        C.ExecuteNonQuery("Update List_ProjectName set Customer_Assistant_ID = '" & Session("uid") & "', Customer_Assistant_Name = '" & Session("ufullname") & "', Area = '" & DT_casst_cluster.Rows(0).Item("Area") & "', Cluster = '" & DT_casst_cluster.Rows(0).Item("Cluster") & "', Cluster_Name = '" & DT_casst_cluster.Rows(0).Item("Cluster_Name") & "', Cluster_Email = '" & DT_casst_cluster.Rows(0).Item("Cluster_Email") & "'  where id_List = '" & hide_doc_no.Value & "'")
                    End If
                Else
                    C.ExecuteNonQuery("Update List_ProjectName set Customer_Assistant_ID = '" & Session("uid") & "', Customer_Assistant_Name = '" & Session("ufullname") & "', Area = '" & DT_casst.Rows(0).Item("Area") & "', Cluster = '" & DT_casst.Rows(0).Item("Cluster") & "', Cluster_Name = '" & DT_casst.Rows(0).Item("Cluster_Name") & "', Cluster_Email = '" & DT_casst.Rows(0).Item("Cluster_Email") & "'  where id_List = '" & hide_doc_no.Value & "'")
                End If
    ...
    Protected Sub GridView1_RowDeleting(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewDeleteEventArgs)
        ...
        If Not CType(GridView1.Rows(e.RowIndex).Cells(1).FindControl("lblID"), Label).Text Is Nothing Then
            ...
            C.ExecuteNonQuery(strSql)
        End If
        ClientScript.RegisterStartupScript(Page.GetType, "", "AlertSuccess('" + hide_doc_no.Value + "');", True)
    ...
    Private Sub CheckData(ByVal SaveOrNext As String)
        If Session("menu") = "upload" And div_UploadProject.Visible = True Then
        ...
        ElseIf Trim(txtCustomerName.Text) = "" Then
            ClientScript.RegisterStartupScript(Page.GetType, "", "AlertNotification('กรุณาระบุ ชื่อลูกค้า');", True)
            txtCustomerName.Focus()
        ElseIf txtCustomerName.Text.Contains("'") = True Then
            ClientScript.RegisterStartupScript(Page.GetType, "", "AlertNotification(""ไม่สามารถใช้ตัว ' ในการกรอกข้อมูล ชื่อลูกค้า ได้"");focus();", True)
            txtCustomerName.Focus()
        ...
        ElseIf getTable_List_CAsst().Rows.Count <= 0 Then
            ClientScript.RegisterStartupScript(Page.GetType, "", "AlertNotification('กรุณาระบุข้อมูล Customer Assistant');", True)
            txtCustomerAssistantName.Focus()
        ElseIf getTable_List_CT().Rows.Count <= 0 Then
            ClientScript.RegisterStartupScript(Page.GetType, "", "AlertNotification('กรุณาระบุข้อมูล Customer Contact');", True)
            txtCustomerContactName.Focus()
        ...
        ElseIf txtDetailProject.Text.Contains("'") = True Then
            ...
            txtDetailProject.Focus()
        ElseIf txtDetailService.Text.Contains("'") = True Then
            ClientScript.RegisterStartupScript(Page.GetType, "", "AlertNotification(""ไม่สามารถใช้ตัว ' ในการกรอกข้อมูล รายละเอียดทางเทคนิค ได้"");focus();", True)
        ...
        ElseIf txtMTTR.Text.Contains("'") = True Then
            ClientScript.RegisterStartupScript(Page.GetType, "", "AlertNotification(""ไม่สามารถใช้ตัว ' ในการกรอกข้อมูล MTTR ได้"");focus();", True)
            txtMTTR.Focus()
    ...
    Public Sub SaveData(ByVal SaveOrNext As String)
        ...
        If Session("menu") = "upload" Then
            ...
            If div_UploadProject.Visible = True Then
                If FileUploadProject.HasFile = True Then
                    ...
                    FileNameProject = "ProjectFile\Project_" + Now.Year.ToString("0000") + Now.Month.ToString("00") + Now.Day.ToString("00") + "_" + Now.Hour.ToString("00") + Now.Minute.ToString("00") + Now.Second.ToString("00") + file_extProject
                    If TheFileProject.Exists Then
                        If System.IO.File.Exists(MapPath(".") & "\Upload\" & FileNameProject) Then
                            System.IO.File.Delete(MapPath(".") & "\Upload\" & FileNameProject)
                        End If
                        System.IO.File.Move(MapPath(".") & "\Upload\" & CurrentFileName, MapPath(".") & "\Upload\" & FileNameProject)
                    Else
                        Throw New FileNotFoundException()
                    End If
                Else
        ...
        If DT_List.Rows.Count > 0 Then
            ...
            strSql += ", Monitor_Time = '" & ddlMonitorTime.SelectedValue & "', Fine = '" & txtFine.Text & "', SpecialApprove = '" & SpecialApprove & "', UpdateBy = '" & hide_user.Value & "', UpdateDate = getdate() " + vbCr
            'If chkSLA.Checked = True Then
            ...
            If Session("menu") = "upload" Then
                If div_UploadProject.Visible = True Then
                    strSql += ", Project_File = '" & FileNameProject.ToString & "' " + vbCr
            ...
            If chkDoc.Checked = True Then
                strSql += ", Doc_File = '" & FileNameDoc.ToString & "' " + vbCr
            End If
            If chkDeleteDoc.Checked = True Then
                strSql += ", Doc_File = '' " + vbCr
            End If
            ...
            Try
                ...
                If SaveOrNext = "Next" Then
                    If Session("menu") = "upload" Then
                        ClientScript.RegisterStartupScript(Page.GetType, "success_update", "AlertSuccess('บันทึกข้อมูลล่าสุดสำเร็จ', function(){ window.location = 'Summary_Upload.aspx?menu=upload'; });", True)
                    Else
                        ClientScript.RegisterStartupScript(Page.GetType, "success_update", "AlertSuccess('บันทึกข้อมูลล่าสุดสำเร็จ', function(){ window.location = 'add_service.aspx?menu=create'; });", True)
                    End If
                Else
                    If Session("menu") = "upload" Then
                        ClientScript.RegisterStartupScript(Page.GetType, "success_update", "AlertSuccess('บันทึกข้อมูลล่าสุดสำเร็จ', function(){ window.location = 'project_name.aspx?menu=upload'; });", True)
                    Else
                        ClientScript.RegisterStartupScript(Page.GetType, "success_update", "AlertSuccess('บันทึกข้อมูลล่าสุดสำเร็จ', function(){ window.location = 'project_name.aspx?menu=create'; });", True)
                    End If

                End If

            Catch ex As Exception
                ClientScript.RegisterStartupScript(Page.GetType, "failed_update", "AlertError('ไม่สามารถบันทึกข้อมูลได้');", True)
            End Try
        ...
        Else
            ...
            strSql += "Status, Special_Price, Upload_Project, Project_File,  " + vbCr
            strSql += "Contract, Monthly, OneTimePayment, MonthlySummary, OneTimeSummary, TotalProject, "
            strSql += "Marketing, EntertainCustomer, Gift, Penalty, PenaltyLate,  " + vbCr
            strSql += "TotalMarketing, TotalEntertainCustomer, TotalGift, TotalPenalty, TotalPenaltyLate,  " + vbCr
            strSql += "ContractGuarantee, SpecialApprove, " + vbCr
            ...
            strSql += "'" & Company.ToString & "','" & TypeOfService.ToString & "','" & txtDetailProject.Text & "','" & txtDetailService.Text & "','" & txtSLA.Text & "','','" & txtMTTR.Text & "',"
            strSql += "'" & ddlMonitorDate.SelectedValue & "','" & ddlMonitorTime.SelectedValue & "','" & txtFine.Text & "','','" & FileNameDoc.ToString & "',"
            strSql += "'1','" & SpecialPrice & "','" & UploadProject & "','" & FileNameProject.ToString & "', "
            strSql += "'" & CDbl(txtContract.Text) & "','" + Format(CDbl(txtInputPrice.Text), "#0.00") + "','" + Format(CDbl(txtOneTimePayment.Text), "#0.00") + "','" + Format(CDbl(lblMonthlyPrice.Text), "#0.00") + "','" + Format(CDbl(lblOneTimePayment.Text), "#0.00") + "','" + Format(CDbl(lblTotalYearly.Text), "#0.00") + "', "
            strSql += "'" + Format(CDbl(txtMarketing.Text), "#0.00") + "','" + Format(CDbl(txtEntertainCustomer.Text), "#0.00") + "','" + Format(CDbl(txtGift.Text), "#0.00") + "','" + Format(CDbl(txtPenalty.Text), "#0.00") + "','" + Format(CDbl(txtPenaltyLate.Text), "#0.00") + "', "
            strSql += "'" + Format(CDbl(lblTotalMarketing.Text), "#0.00") + "','" + Format(CDbl(lblTotalEntertainCustomer.Text), "#0.00") + "','" + Format(CDbl(lblTotalGift.Text), "#0.00") + "','" + Format(CDbl(lblTotalPenalty.Text), "#0.00") + "','" + Format(CDbl(lblTotalPenaltyLate.Text), "#0.00") + "', "
            strSql += "'" + Format(CDbl(txtContractGuarantee.Text), "#0.00") + "','" & SpecialApprove & "' ,"
            strSql += "'" & hide_user.Value & "',getdate(),'" & hide_user.Value & "',getdate())"
            Try
                'Response.Write(strSql)
                C.ExecuteNonQuery(strSql)
                ...
                If SaveOrNext = "Next" Then
                    If Session("menu") = "upload" Then
                        ClientScript.RegisterStartupScript(Page.GetType, "success_insert", "AlertSuccess('บันทึกข้อมูลได้สำเร็จ', function(){ window.location = 'Summary_Upload.aspx?menu=upload'; });", True)
                    Else
                        ClientScript.RegisterStartupScript(Page.GetType, "success_insert", "AlertSuccess('บันทึกข้อมูลได้สำเร็จ', function(){ window.location = 'add_service.aspx?menu=create'; });", True)
                    End If
                Else
                    If Session("menu") = "upload" Then
                        ClientScript.RegisterStartupScript(Page.GetType, "success_insert", "AlertSuccess('บันทึกข้อมูลได้สำเร็จ', function(){ window.location = 'project_name.aspx?menu=upload'; });", True)
                    Else
                        ClientScript.RegisterStartupScript(Page.GetType, "success_insert", "AlertSuccess('บันทึกข้อมูลได้สำเร็จ', function(){ window.location = 'project_name.aspx?menu=create'; });", True)
                    End If
                End If
    ...
    '        strSql += "Marketing, EntertainCustomer, Gift, Penalty, PenaltyLate,  " + vbCr
    '        strSql += "TotalMarketing, TotalEntertainCustomer, TotalGift, TotalPenalty, TotalPenaltyLate,  " + vbCr
    '        strSql += "CreateBy, CreateDate)"
    '        strSql += "values ('" & CDbl(txtContract.Text) & "','" + Format(CDbl(txtInputPrice.Text), "#0.00") + "','" + Format(CDbl(txtOneTimePayment.Text), "#0.00") + "','" + Format(CDbl(lblMonthlyPrice.Text), "#0.00") + "','" + Format(CDbl(lblOneTimePayment.Text), "#0.00") + "','" + Format(CDbl(lblTotalYearly.Text), "#0.00") + "', "
    '        strSql += "'" + Format(CDbl(txtMarketing.Text), "#0.00") + "','" + Format(CDbl(txtEntertainCustomer.Text), "#0.00") + "','" + Format(CDbl(txtGift.Text), "#0.00") + "','" + Format(CDbl(txtPenalty.Text), "#0.00") + "','" + Format(CDbl(txtPenaltyLate.Text), "#0.00") + "', "
    '        strSql += "'" + Format(CDbl(lblTotalMarketing.Text), "#0.00") + "','" + Format(CDbl(lblTotalEntertainCustomer.Text), "#0.00") + "','" + Format(CDbl(lblTotalGift.Text), "#0.00") + "','" + Format(CDbl(lblTotalPenalty.Text), "#0.00") + "','" + Format(CDbl(lblTotalPenaltyLate.Text), "#0.00") + "', "
    '        strSql += "'" + Session("uemail") + "',getdate()) "
    '        C.ExecuteNonQuery(strSql)
    '    End If
    'End Sub
    ...
    Public Sub CalculateSellingPrice()
        ...
        Else
            sMenu = "Create"
        End If
        DT_GetTableProjectName = Cal.GetTableService(Session("uemail"), sMenu, "")
        DT_GetTableService = Cal.GetTableService(Session("uemail"), sMenu, "")

        DT_CAPEX = Cal.ShowCAPEX_OPEX(Session("uemail"), sMenu, "", DT_GetTableService, ddlCustomerType.SelectedValue)
        If DT_CAPEX.Rows.Count > 0 Then
            DT_Service = Cal.ShowService(Session("uemail"), sMenu, "", DT_GetTableProjectName, DT_GetTableService, CInt(txtContract.Text), CDbl(txtInputPrice.Text), CDbl(txtOneTimePayment.Text), CDbl(txtMarketing.Text), CDbl(txtEntertainCustomer.Text), CDbl(txtPenalty.Text), CDbl(txtPenaltyLate.Text), CDbl(txtGift.Text), DT_CAPEX.Rows(0).Item("TotalCapex"), DT_CAPEX.Rows(0).Item("TotalOpex"), DT_CAPEX.Rows(0).Item("TotalOther"), DT_CAPEX.Rows(0).Item("TotalManagement"))
        End If
    ...
    Public Sub ClearData()
        Dim DS_List As New DataTable
        DS_List = C.GetDataTable("SELECT id_List as ScopeIdentity from List_ProjectName where CreateBy = '" & Session("uemail") & "' and Document_No is null ")
        If DS_List.Rows.Count > 0 Then
            Dim ProjectName_Id As String = DS_List.Rows(0).Item("ScopeIdentity").ToString()
            C.ExecuteNonQuery("delete from tmpList_CAsst where ProjectName_id = '" + ProjectName_Id + "' ")
            C.ExecuteNonQuery("delete from tmpList_CT where ProjectName_id = '" + ProjectName_Id + "' ")
        End If
        C.ExecuteNonQuery("delete from List_ProjectName where CreateBy = '" & Session("uemail") & "' and Document_No is null")
        ClientScript.RegisterStartupScript(Page.GetType, "success_clear", "window.location = 'project_name.aspx?menu=" & Session("menu") & "';", True)
    End Sub
